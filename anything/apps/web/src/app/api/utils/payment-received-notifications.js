...
export async function sendPaymentReceivedNotifications({
  ...
  if (invoice?.paid_via_credits === true) {
    return {
      ...
      advertiser_email_error: null,
      internal_notifications: null,
    };
  }

  if (normalizeStatus(invoice.status) !== "paid") {
    throw new Error("Invoice is not marked as paid yet.");
  }
  ...
  if (!advertiserEmail) {
    return {
      skipped: true,
      reason: "missing_advertiser_email",
      advertiser_email_sent: false,
      advertiser_email_error: "Advertiser email is missing on this invoice.",
      internal_notifications: null,
    };
  }
...
`;
  ...
  try {
    ...
    console.error("[payment-received-notifications] Advertiser email failed:", error);
  }
